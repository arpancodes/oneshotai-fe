...
function Plot({resource}) {
  ...
  useEffect(() => {
    fetch(`${API}/colleges/stats/${resource}`)
      ...
      .then((res) => {
        const data = {datasets: [{label: `By ${resource}`, borderWidth: 1,}]}
        data.labels = res.data.colleges.map(college => college._id)
        data.datasets[0].data = res.data.colleges.map(college => college.count)
        const colors = res.data.colleges.map(_ => randomRGBColor())
        data.datasets[0].backgroundColor = colors.map(color => `rgba(${color}, 0.5)`)
        data.datasets[0].borderColor = colors.map(color => `rgba(${color}, 1)`)
        setData(data);
        console.log(data);
        setLoading(false);
      });
